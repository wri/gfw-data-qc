...
parser.add_argument('--output', type=str, help='path to output directory', default='out')
parser.add_argument('--start', type=str, help='start date in ISO (YYYY-MM-DD) format')
parser.add_argument('--end', type=str, help='END date in ISO (YYYY-MM-DD) format')
...
print('Removing confidence encoding')
arr[arr >= 30000] -= 30000
arr[arr >= 20000] -= 20000
...
print('Intersecting with contextual layers')
feature = intersect_layers(bounds=bounds, shp_fp=shp_fp, layers=contextual_layers)
tmp_fp=os.path.join('tmp', f'{os.path.basename(shp_fp[:-4])}_dissolved.shp')
feature.to_file(tmp_fp)

# sum number of alerts
print('Calculating number of alerts')
zstats = zonal_stats(
    tmp_fp,
    arr_mask,
    stats='sum',
